...
const impl = {
  async create(request: Request, env: any, url: URL) {
  ...
  async directUpload(request: Request, env: any, url: URL) {
    ...
    const meta = await env.KV_CONFIG.get(`upload:pending:${token}`, 'json') as any
    if (!meta) {
