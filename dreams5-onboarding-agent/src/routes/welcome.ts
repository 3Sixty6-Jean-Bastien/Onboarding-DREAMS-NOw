...
type WelcomeBody = { tenantId: string; email?: string }

const impl = {
  async send(request: Request, env: any) {
    const raw = await request.json().catch(() => ({})) as Partial<WelcomeBody>
    const { tenantId, email } = raw
    if (!tenantId) return new Response(JSON.stringify({ error: 'tenantId required' }), { status: 400, headers: { 'content-type': 'application/json' } })
...
export async function handle(request: Request, env: any) {
  if (request.method === 'POST') return impl.send(request, env)
