...
function getTenantId(evt: any): string | undefined {
  try {
    if (evt?.data?.object?.metadata?.tenant_id) return String(evt.data.object.metadata.tenant_id)
    if (evt?.data?.object?.metadata?.tenantId) return String(evt.data.object.metadata.tenantId)
    if (evt?.tenantId) return String(evt.tenantId)
  } catch {}
  return undefined
}
...
export async function handle(request: Request, env: any) {
  ...
  if (!tenantId) return new Response(JSON.stringify({ ok: false, error: 'missing tenant id' }), { status: 400, headers: { 'content-type': 'application/json' } })
  ...
  try {
    ...
    const steps: string[] = cfg?.steps?.map((s: any) => s.key) || ['setup-domain','connect-ga4']
