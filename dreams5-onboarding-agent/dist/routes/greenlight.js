...
function toNumber(v) { const n = v ? parseInt(v, 10) : NaN; return Number.isFinite(n) ? n : undefined; }
export async function handle(request, env) {
    const url = new URL(request.url);
    const tenantId = url.searchParams.get('tenant_id') || url.searchParams.get('tenantId') || '';
    if (!tenantId)
        return new Response(JSON.stringify({ error: 'tenant_id required' }), { status: 400, headers: { 'content-type': 'application/json' } });
    const cfg = await getConfig(env.KV_CONFIG).catch(() => null);
    const requiredSteps = (cfg?.steps?.map((s) => s.key) || ['setup-domain', 'connect-ga4']).concat(['payment']);
    try {
        ...
        for (const row of (res.results || [])) {
            if (!latest.has(row.step_key))
                latest.set(row.step_key, row.status);
        }
        const satisfied = new Set();
        for (const key of latest.keys()) {
            const st = latest.get(key);
            if (st && ['verified', 'completed', 'succeeded', 'stored'].includes(st))
                satisfied.add(key);
        }
